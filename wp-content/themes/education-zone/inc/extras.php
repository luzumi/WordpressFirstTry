...
function education_zone_body_classes( $classes ) {
    ...
    $ed_banner   = get_theme_mod( 'education_zone_ed_slider_section', false );

	// Adds a class of group-blog to blogs with more than 1 published author.
    if( ( $my_theme['Name'] == 'Education Zone' && $theme_style =='modern' ) || ( $my_theme['Name'] == 'Smart Education' && $theme_style_se =='modern' ) || ( $my_theme['Name'] == 'High School' && $theme_style_hs =='modern' )) {
    ...
    ?>
        <?php if( has_custom_logo() || $site_title || $site_description || $header_text ){?>
            <div class="site-branding" itemscope itemtype="https://schema.org/Organization">
                ...
                if( $header_text ){ 
                        if ( is_front_page() ) : 
                            echo '<div class="site-branding-wrapper">'; ?>
...
function education_zone_fonts_url() {
    ...
    */
    ...
    $figtree = _x( 'on', 'Figtree: on or off', 'education-zone' );
    ...
    */
    ...
    if( 'off' !== $roboto && $theme_style == 'classic' ){
        $font_families[] = 'Roboto:400,700';
    }

    if( 'off' !== $lato && $theme_style == 'classic' ){
        $font_families[] = 'Lato:400,700,900';
    }

    if( ( $theme_style == 'modern' && 'off' !== $figtree )){
        $font_families[] = 'Figtree:400,400italic,700italic,700';
    }
        ...
        <div class="site-info">
            ...
            <?php 
                ...
                <span class="by">
                    ...
                    <a rel="nofollow" href="<?php echo esc_url( 'https://rarathemes.com/' ); ?>" target="_blank"><?php echo esc_html__( 'Rara Themes', 'education-zone' ); ?></a>.
...
if( ! function_exists( 'education_zone_load_preload_local_fonts') ) :
/**
 ...
 */
function education_zone_load_preload_local_fonts( $url, $format = 'woff2' ) {

    // Check if cached font files data preset present or not. Basically avoiding 'education_zone_WebFont_Loader' class rendering.
    $local_font_files = get_site_option( 'education_zone_local_font_files', false );

    if ( is_array( $local_font_files ) && ! empty( $local_font_files ) ) {
        $font_format = apply_filters( 'education_zone_local_google_fonts_format', $format );
    ...
    $font = education_zone_webfont_loader_instance( $url );
    $font->set_font_format( $format );
    $font->preload_local_fonts();
}
endif;

if( ! function_exists( 'education_zone_flush_local_google_fonts' ) ){
    /**
